...
static bool g_isInBottomPose = false;
        ...
        if (infFramebuf)
        {
			...
			// ---- EXERCISE CLASSIFIER AND REP COUNTER ----
					  ...
					  float current_pose_conf = 0.0f;
						float prob_standing = 0.0f;
						float prob_bottom = 0.0f;
            ...
            if (infFramebuf->results.size() > 0)
            {
                ...
                // Extract INT8 outputs and Dequantize (It's now a true percentage!)
                int8_t* repOut = (int8_t*)repOutTensor->data.data;

								prob_standing   = ((int)repOut[1] - repOutQ.offset) * repOutQ.scale / 2 * 10 + 0.125;
								prob_bottom = ((int)repOut[0] - repOutQ.offset) * repOutQ.scale / 7 * 10 - 0.05;

                // Pick the highest probability
                int current_pose_class = (prob_standing > prob_bottom) ? 1 : 0;
                current_pose_conf = (current_pose_class == 1) ? prob_standing : prob_bottom;

                // 3. State Machine Logic
                if (current_pose_conf >= 0.75f) {
                    if (current_pose_class == 0) {
                        g_isInBottomPose = true; 
                    } else if (current_pose_class == 1 && g_isInBottomPose) {
                        g_repCount++;            
                        g_isInBottomPose = false;	
                    }
                }
							}
            ...
            Display_FillRect((uint16_t *)infFramebuf->frameImage.data, &sDispRect, IMAGE_DISP_UPSCALE_FACTOR);
                    ...
                    char repText[64];
                    const char* pose_name = g_isInBottomPose ? "DOWN" : "UP";
							snprintf(repText, sizeof(repText), "Reps: %d [%s] (stand: %f, bottom %f)", g_repCount, pose_name, prob_standing, prob_bottom);
                    Display_PutText_Wrapped(
                            repText,
                            650,
                            FONT_HTIGHT*2 + 100, // Placed slightly underneath the Exercise text
                            C_RED,
                            C_WHITE,
